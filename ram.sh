...
echo -e "$verd Digite LISTA para ver meus comandos disponiveis, ou execute com o parametro -l. $color "; sleep 1s
	if [[ $USER != "root" ]]
	then
	echo -e "$verm Hey $USER você não esta logado como Root $color"
	fi
...
case $texto in
   ...
   *"mostre arquivos"*|*"mostra os arquivos"*)
   echo -e "$verd Arquivos de qual local?  $color" 
   read -p "Digite o local que deseja listar (ex: /root): " listt
   ls --color $listt ; sleep 2s
   ;;
esac
done
